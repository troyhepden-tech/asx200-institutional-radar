...
st.caption(
    "V7 — change-notice-first scanning with reconstructed 604 tables and stricter validation."
)
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "V7 prioritises Form 604 change notices, reconstructs flattened voting tables, "
        "and quarantines ambiguous results instead of guessing."
    )
...
if deep_scan and not df.empty:
    st.session_state["parsed_filings_v7"] = {}
    priority = {"CHANGE": 0, "CROSSED 5%+": 1, "FELL BELOW 5%": 2}
    targets = (
        df.assign(_scan_priority=df["Disclosure"].map(priority).fillna(9))
    ...
    progress.empty()
    st.session_state["parsed_filings_v7"] = parsed

parsed = st.session_state.get("parsed_filings_v7", {})
