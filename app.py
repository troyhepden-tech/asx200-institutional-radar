...
st.caption(
    "V9 — coordinate-aware PDF table parsing, stronger holder identification, and safer confidence scoring."
)
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "V9 adds coordinate-aware PDF word parsing for stubborn Form 604 tables, stronger holder-name scoring, "
        "and dedicated TR-1 extraction for foreign major-holding notices."
    )
...
if (deep_scan or scan_all_asx200) and not df.empty:
    st.session_state["parsed_filings_v9"] = {}
    ...
    progress.empty()
    st.session_state["parsed_filings_v9"] = parsed

parsed = st.session_state.get("parsed_filings_v9", {})
