...
def clean_holder(value):
    ...
    if "if applicable" in low:
        return None
    if low.startswith(("to company name/scheme", "company name/scheme")):
        return None
...
def extract_tr1_position(text):
    """
    TR-1: extract current and previous percentages from their own labelled
    sections. Never convert total issuer voting rights into holder shares.
    """
    head = text[:30000]
    low = head.lower()

    cur_pos = low.find("resulting situation on the date on which threshold")
    prev_pos = low.find("position of previous notification")

    current = previous = None

    if cur_pos >= 0:
        end = prev_pos if prev_pos > cur_pos else min(len(head), cur_pos + 5000)
        block = head[cur_pos:end]
        vals = [
            to_float(x)
            for x in re.findall(r"(\d{1,2}(?:\.\d{2,6})?)\s*%", block)
        ]
        vals = [x for x in vals if x is not None and 0 < x < 100]
        if vals:
            # TR-1 often repeats direct/indirect/total. The final repeated
            # meaningful percentage is normally the total current position.
            current = vals[-1]

    if prev_pos >= 0:
        block = head[prev_pos:min(len(head), prev_pos + 2500)]
        vals = [
            to_float(x)
            for x in re.findall(r"(\d{1,2}(?:\.\d{2,6})?)\s*%", block)
        ]
...
st.caption(
    "V8 — full ASX 200 deep scan, stronger holder extraction, and multi-layout 604/TR-1 parsing."
)
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "V8 can scan every ASX 200 disclosure in the current feed, uses multiple 604/TR-1 layouts, "
        "and cleans issuer/form labels out of holder names."
    )
...
if (deep_scan or scan_all_asx200) and not df.empty:
    st.session_state["parsed_filings_v8"] = {}
    ...
    progress.empty()
    st.session_state["parsed_filings_v8"] = parsed

parsed = st.session_state.get("parsed_filings_v8", {})
