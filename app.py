...
def extract_604_row(text):
    """
    A Form 604 voting row is semantically:
      class | PREVIOUS votes | PREVIOUS % | PRESENT votes | PRESENT %
    We only accept a row if the four values appear in that order.
    This prevents HLI-style percentage reversal and annexure numbers leaking in.
    """
    seg = voting_segment(text)

    # Strongest pattern: two large vote counts, each immediately followed by a %.
    pattern = re.compile(
        r"(?<![\d.])(\d{1,3}(?:,\d{3})+|\d{5,})\s+"
        r"(\d{1,2}(?:\.\d{1,6})?)\s*%?\s+"
        r"(\d{1,3}(?:,\d{3})+|\d{5,})\s+"
    ...
    for m in pattern.finditer(seg):
        pv = to_int(m.group(1))
        pp = to_float(m.group(2))
        cv = to_int(m.group(3))
        cp = to_float(m.group(4))

        if not all(x is not None for x in [pv, pp, cv, cp]):
            continue
        if not (0 < pp < 100 and 0 < cp < 100):
            continue
    ...
    # PyMuPDF sometimes places headers between the numbers.
    # Strip known table/header words but preserve number order.
    compact = re.sub(
        r"(?i)\b(previous|present|notice|person'?s?|votes?|voting|power|class|of|securities|fpo|ordinary|shares?|fully|paid)\b",
        " ",
        seg,
    )
    ...
    for m in pattern.finditer(compact):
        ...
        cp = to_float(m.group(4))
        if (
            pv and cv and pp and cp
            and 0 < pp < 100 and 0 < cp < 100
            and pv >= 1000 and cv >= 1000
        ):
            return pv, pp, cv, cp
...
def extract_tr1_position(text):
    """
    TR-1 has percentages plus total issuer voting rights, not necessarily
    the holder's vote count. Do NOT treat issuer total as holder shares.
    """
    head = text[:22000]
    ...
    m = re.search(
        r"Resulting situation.{0,1200}?(\d{1,2}(?:\.\d{2,6})?)\s*%",
        head,
        re.I | re.S,
    )
    if m:
        current = to_float(m.group(1))

    m = re.search(
        r"Position of previous notification.{0,600}?(\d{1,2}(?:\.\d{2,6})?)\s*%",
        head,
        re.I | re.S,
    )
    if m:
        previous = to_float(m.group(1))
...
st.caption(
    "V6 — validated table parsing: impossible share/% combinations are blocked from the radar."
)
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "V6 uses label-aware table extraction plus mathematical validation. "
        "Bad pairings are quarantined instead of becoming green/red signals."
    )
...
if deep_scan and not df.empty:
    st.session_state["parsed_filings_v6"] = {}
    ...
    progress.empty()
    st.session_state["parsed_filings_v6"] = parsed

parsed = st.session_state.get("parsed_filings_v6", {})
